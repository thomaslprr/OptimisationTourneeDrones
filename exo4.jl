...
function solve_CW(nom_fichier::String)
	...
	for i in 1:size(listeCouple,1)
    	...
    	if indice1 === nothing && indice2 === nothing && verifDemande([listeCouple[i][1]],[listeCouple[i][2]],listeDemande,capacite) #Condition si les 2 clients du couple i ne sont pas dans la liste des tournées
        	push!(resultat, [listeCouple[i][1],listeCouple[i][2]])
    	elseif indice1 !== nothing && indice2 === nothing && verifDemande(resultat[indice1],[listeCouple[i][2]],listeDemande,capacite) #Condition si le premier client du couple i est dans la liste des tournées mais pas le deuxième
		...
		elseif indice1 === nothing && indice2 !== nothing && verifDemande(resultat[indice2],[listeCouple[i][1]],listeDemande,capacite) #Condition si le deuxième client du couple i est dans la liste des tournées mais pas le premier
		...
		elseif indice1 !== nothing && indice2 !== nothing && indice1 != indice2 && verifDemande(resultat[indice1],resultat[indice2],listeDemande,capacite) #Condition si les 2 clients du couple i sont dans la liste des tournées
			if resultat[indice1][1] == listeCouple[i][1] && resultat[indice2][1] == listeCouple[i][2]
				reverse!(resultat[indice1])
				for j in 1:size(resultat[indice2],1)
					push!(resultat[indice1],resultat[indice2][j])
				end
			...
			elseif resultat[indice1][1] == listeCouple[i][1] && resultat[indice2][size(resultat[indice2],1)] == listeCouple[i][2]
				for j in 1:size(resultat[indice1],1)
					push!(resultat[indice2],resultat[indice1][j])
				end
			...
			elseif resultat[indice1][size(resultat[indice1],1)] == listeCouple[i][1] && resultat[indice2][size(resultat[indice2],1)] == listeCouple[i][2]
				...
				for j in 1:size(resultat[indice1])
					push!(resultat[indice2],resultat[indice1][j])
				end
				deleteat!(resultat,indice1)
			end
...
function verifDemande(tab1::Array{Int64}, tab2::Array{Int64}, listeDemande::Array{Int64}, limite::Int64)
	resultat::Int64 = 0
